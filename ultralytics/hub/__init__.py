...
def start(key=""):
    ...
    Usage: from ultralytics.hub import start; start('API_KEY')
    """
    auth = Auth(key)
...
def export_model(key="", format="torchscript"):
    ...
    api_key, model_id = split_key(key)
    r = requests.post(
        "https://api.ultralytics.com/export", json={"apiKey": api_key, "modelId": model_id, "format": format}
    )
...
def get_export(key="", format="torchscript"):
    ...
    api_key, model_id = split_key(key)
    r = requests.post(
        "https://api.ultralytics.com/get-export", json={"apiKey": api_key, "modelId": model_id, "format": format}
    )
